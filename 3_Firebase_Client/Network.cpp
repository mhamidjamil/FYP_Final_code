...
void Network::firestoreDataUpdate_1(String user_ID, double temp,
                                    String position, double conductance,
                                    double resistance,
                                    double resistive_voltages,
                                    double snore_voltages) {
  if (WiFi.status() == WL_CONNECTED && Firebase.ready()) {
    ...
    Serial.print(" _ ");
    Serial.print(resistive_voltages);
    Serial.print(" _ ");
    Serial.print(snore_voltages);
    Serial.println(").");
    //    String documentPath = "Patient/patient_1";
    String documentPath = ("users/" + user_ID).c_str();
